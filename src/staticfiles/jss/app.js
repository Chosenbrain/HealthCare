...
function myjsapp(peerClient) {
    ...
    function EventListeners() {
        ...
        $("#user-name").keypress(function(event) {
            if (13 == event.which) {
                ...
                if (cookie.get("username") != username) startPeerClient(username);
            }
        });
    ...
    function appendToHistory(id, message, isSent) {
        if (chatHistory[id]) {
            ...
            var msg = $("<li><b>" + fromTxt + ": </b></li>").append(
                "<span>" + message + "</span"
            );
    ...
    return {
        ...
        createChatWindow: function(id) {
            var toPeerId = id;
            var panel = $(
                '<div class="panel panel-primary chat-div"><div class="panel-heading"></div>' +
            ...
            var videoCall = $(
                '<i class="fa fa-video-camera fa-2x call-icon" aria-hidden="true"></i>'
            );
            var audioCall = $(
            ...
            $(".panel-body", panel)
                .append('<span class="text-primary">You are both online</span>')
                .append(history);
        ...
        showVideoCall: function(options) {
            ...
            if (options["video"]) $("#videoCallPanel .title").text("Calling");
            else $("#videoCallPanel .title").text("Calling");
        },
        showIncomingCall: function(peerId, options) {
            $("#callConfirmationModal").modal("show");
            if (options["video"]) var txt = "Incoming Video call from : " + peerId;
            else var txt = "Incoming Voice call from : " + peerId;
